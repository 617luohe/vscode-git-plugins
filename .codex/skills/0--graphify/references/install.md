...
Step 1 专用。检测已安装的 graphify，未安装则自动安装，保存 Python 解释器路径供后续步骤使用。
...
如果 import 成功，不打印任何输出，直接进入 Step 2。

**每个后续步骤中，使用保存的解释器路径运行 Python — `& (Get-Content graphify-out\.graphify_python)` 替代裸 `python3` — 确保所有步骤使用实际安装了 graphify 的解释器。**
